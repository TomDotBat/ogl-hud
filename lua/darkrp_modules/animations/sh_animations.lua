...
local function physGunCheck(ply)
    ...
    hook.Add("Think", hookName, function()
        if IsValid(ply) and ply:Alive() and ply:GetActiveWeapon():IsValid() and ply:GetActiveWeapon():GetClass() == "weapon_physgun" and ply:KeyDown(IN_ATTACK) then --{{ user_id sha256 key }}
...
local function KeysAnims(um)
    ...
    ply:AnimRestartGesture(GESTURE_SLOT_CUSTOM, act == "usekeys" and ACT_GMOD_GESTURE_ITEM_PLACE or ACT_HL2MP_GESTURE_RANGE_ATTACK_FIST, true) --{{ user_id }}
end
...
local function AnimationMenu()
    ...
    local Height = table.Count(Anims) * 52 + 46
    AnimFrame:SetSize(220, Height)
    AnimFrame:SetPos(ScrW() / 2 + ScrW() * 0.1, ScrH() / 2 - (Height / 2))
    AnimFrame:SetTitle("")
    AnimFrame.btnMaxim:SetVisible(false)
    AnimFrame.btnMinim:SetVisible(false)
    AnimFrame.btnClose:SetVisible(false)

    local closebtn = vgui.Create("DButton", AnimFrame)
    closebtn:SetText("")
    closebtn:SetPos(189, 7)
    ...
    function AnimFrame:Paint(w, h)
        ...
        draw.SimpleText("ANIMATIONS", "OGL.HUD.DermaTitle", 12, 17, OGLFramework.UI.ColourScheme.lightText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)--{{ script_version_name }}
    end
    ...
    for k, v in SortedPairs(Anims) do
        ...
        button:SetPos(10, (i - 1) * 52 + 46)
        button:SetSize(200, 42)
        button:SetText("")
        button.text = v
        ...
        function button:Paint(w, h)
            ...
            draw.SimpleText(self.text, "OGL.HUD.Button", w/2, h * .47, OGLFramework.UI.ColourScheme.lightText, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    end
end
